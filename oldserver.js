...
app.get('/api/pages', authenticateToken, (req, res) => {
    ...
    if (role === 'admin' || role === 'superadmin') {
        ...
        const pages = [
            ...
            // Reporting Section
            { name: "Handlers Reporting", file: "/admin/handlers-reporting", icon: "fa-solid fa-file-lines", badge: 1 },
            { name: "staff Reporting", file: "/admin/staff-reporting", icon: "fa-solid fa-file-lines", badge: 1 },
...
app.get('/api/complaint-center/get-all-complaints', authenticateToken, async (req, res) => {
    try {
        ...
        const formattedComplaints = complaints.map(c => ({
            ...
            // ✅ FIXED: Category with proper type
            category: {
                id: c.nature_id,
            ...
            // ✅ FIXED: For easy access in table
            ...
            // ✅ FIXED: Resolver with designation as string
            resolver: c.staff_id ? {
                id: c.staff_id,
                name: c.staff_name,
                email: c.staff_email,
                ...
                designation: c.staff_designation || 'staff' // String, not object
            } : null,

            // ✅ FIXED: Updated By - Shows who last updated
    ...
    try {
        ...
        const [buildings] = await pool.query(`
            ...
            LEFT JOIN (
                ...
                ON floor_counts.building_id = b.id
            ...
            LEFT JOIN (
                ...
                ON room_counts.building_id = b.id

            ORDER BY b.id DESC;
        `);

        // Convert picture paths to relative URLs
        const buildingsWithImageUrls = buildings.map(building => ({
            ...building,
            picture: building.picture ? `/assets/buildings/${building.picture}` : null
        }));

        res.json({
            success: true,
            data: buildingsWithImageUrls
        });
    } catch (error) {
...
app.get('/api/buildings/:id', async (req, res) => {
    try {
        ...
        const building = {
            ...buildings[0],
            picture: buildings[0].picture ? `/assets/buildings/${buildings[0].picture}` : null
        };
...
app.post('/api/buildings', buildingUpload.single('image'), async (req, res) => {
    try {
        ...
        const buildingResponse = {
            ...newBuilding[0],
            picture: newBuilding[0].picture ? `/assets/buildings/${newBuilding[0].picture}` : null
        };
    ...
    try {
        ...
        if (req.file) {
            ...
            if (existingBuilding[0].picture) {
                const oldImagePath = path.join(__dirname, 'assets', 'buildings', existingBuilding[0].picture);
        ...
        const buildingResponse = {
            ...updatedBuilding[0],
            picture: updatedBuilding[0].picture ? `/assets/buildings/${updatedBuilding[0].picture}` : null
        };
...
app.delete('/api/buildings/:id', async (req, res) => {
    try {
        ...
        // Check if building exists and get image filename
        const [existingBuilding] = await pool.query(
        ...
        if (existingBuilding[0].picture) {
            const imagePath = path.join(__dirname, 'assets', 'buildings', existingBuilding[0].picture);
            if (fs.existsSync(imagePath)) {
                fs.unlinkSync(imagePath);
            }
        }
...
app.get('/api/floors-management/floors/:buildingId', async (req, res) => {
    try {
        ...
        const floorsWithImages = floors.map(floor => ({
            ...floor,
            picture: floor.picture ? `/assets/floors/${floor.picture}` : null
        }));
        ...
        data.forEach(row => {
            ...
            if (row.floor_id && row.building_id) {
                structuredData[row.colony_id].buildings[row.building_id].floors[row.floor_id] = {
                    id: row.floor_id,
                    name: row.floor_name,
                    picture: row.floor_picture ? `/assets/floors/${row.floor_picture}` : null,
...
app.get('/api/floors-management/floor-details/:floorId', async (req, res) => {
    try {
        ...
        const floorData = {
            ...floorDetails[0],
            picture: floorDetails[0].picture ? `/assets/floors/${floorDetails[0].picture}` : null,
            rooms: rooms
        };
...
app.post('/api/floors-management/floors', floorUpload.single('image'), async (req, res) => {
    try {
        ...
        const floorResponse = {
            ...newFloor[0],
            picture: newFloor[0].picture ? `/assets/floors/${newFloor[0].picture}` : null
        };
...
app.put('/api/floors-management/floors/:id', floorUpload.single('image'), async (req, res) => {
    try {
        ...
        const floorResponse = {
            ...updatedFloor[0],
            picture: updatedFloor[0].picture ? `/assets/floors/${updatedFloor[0].picture}` : null
        };
